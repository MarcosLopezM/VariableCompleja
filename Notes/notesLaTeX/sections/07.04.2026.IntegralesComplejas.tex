...
\begin{definition}
	...
	\begin{equation}
		\int_{a}^{b}\odif[sep-end=\medspace]{t} f(t) = \int_{a}^{b}\odif[sep-end=\medspace]{t}
...
\begin{property}
	...
	\begin{enumerate}
		\item \(\Re \Bigl(\int_{a}^{b}\odif[sep-end=\medspace]{t} f(t)\Bigr) =
		      ...
		      \Im \Bigl(\int_{a}^{b}\odif[sep-end=\medspace]{t} f(t)\Bigr) = \int_{a}^{b}\odif[sep-end=\medspace]{t} \Im(f(t)).\)
...
\begin{proof}
	\begin{enumerate}[start=3]
		\item Sea \(\lambda = \int_{a}^{b}\odif[sep-end=\medspace]{t} f(t)\). Si
		      ...
		      \abs{\lambda}\mathrm{e}^{i\theta}\), donde el argumento principal es \(\theta =
		      \arg(\lambda)\in (-\pi, \pi]\). Despejando \(\abs{\lambda} = \lambda
		      \mathrm{e}^{-i\theta}\),

		      \begin{align}
			      \abs`\Bigg{\int_{a}^{b}\odif[sep-end=\medspace]{t} f(t)} = \abs{\lambda} & =
			      \Re(\abs{\lambda})  = \Re \bigl(\lambda \mathrm{e}^{-i\theta}\bigr),\nonumber                                                    \\
			                                                                               & = \Re \Bigl(\int_{a}^{b}\odif[sep-end=\medspace]{t}
			      \mathrm{e}^{-i\theta}f(t)\Bigr) =
			      \int_{a}^{b}\odif[sep-end=\medspace]{t} \Re(\mathrm{e}^{-i\theta}) ,\nonumber                                                    \\
			      ...
			      \underbrace{\abs{\mathrm{e}^{-i\theta}}}_{= 1}\abs{f(t)},\nonumber                                                               \\
			                                                                               & = \int_{a}^{b}\odif[sep-end=\medspace]{t} \abs{f(t)}.
		      \end{align}
	\end{enumerate}
\end{proof}
...
\begin{definition}
	...
	\(\eval{\gamma}{\bigl[t_{i-1}, t_{i}\bigr]}\) es de clase \(C^{1}\) \(\forall i \in \{1,
	...
	\begin{figure}[htb]
		\centering
		\includestandalone[width=0.5\linewidth]{figs/suave-a-pedazos}
	\end{figure}
\end{definition}

\begin{definition}
	Sean \(\gamma \colon [a,b] \to \mathbb{C}\) y \(\delta \colon [c,d]  \to \mathbb{C}\)
	curvas suaves por pedazos tales que \(\gamma(b) = \delta(c)\). Definimos la
	concatenación de \(\gamma\) y \(\delta\) denotada por \(\gamma + \delta (\gamma \star
	\delta)\), como
